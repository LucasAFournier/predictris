...
class PlotMetrics:
    ...
    def __init__(
        ...
        self.colors = (
            plt.cm.tab10(np.linspace(0, 1, len(data_series)))
            if data_series
            else []
        )
    ...
    def plot(self, metrics_to_plot: List[str]):
        ...
        for i, metric in enumerate(metrics_to_plot):
            ...
            if metric in self.metric_plotters:
                self.metric_plotters[metric](
                    ax, metric, **self.metric_configs.get(metric, {})
                )
            if i < num_metrics - 1:
    ...
    def _plot_distribution_over_steps(
        self,
        ax: plt.Axes,
        metric: str,
        ylabel: str = "",
        ...
        **kwargs,
        ...
        ref_data = next(
            (
                run_data.get(metric)
                for run_data in self.data_series
                if run_data.get(metric)
            ),
            None,
        )
        ...
        step_delta = (
            np.diff(total_steps).mean() if len(total_steps) > 1 else 1.0
        )
...
def plot_error_rates(
    ...
    max_x = 0
    for (total_steps, all_reps_error_rates), color, legend in zip(
        zip(total_steps_data, error_rates_data), colors, legends
    ):
        ...
        ax.fill_between(
            total_steps, mean - std, mean + std, color=color, alpha=0.2
        )
