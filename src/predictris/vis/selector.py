...
class HtmlGenerator:
    ...
    def generate_html(
        num_trees: int,
        total_nodes: int,
        total_level_hist: str,
        total_confidence_hist: str,
        tree_rows: str,
    ) -> str:
        """Generate the complete HTML page."""
        with open(
            __file__.replace("selector.py", "html/selector.html"), "r"
        ) as f:
            template = f.read()
    ...
    @staticmethod
    def generate_tree_row(
        tree_name: str, display_name: str, metrics: dict, pred_image: str
    ) -> str:
        """Generate HTML for a single tree row."""
        with open(
            __file__.replace("selector.py", "html/tree_row.html"), "r"
        ) as f:
            template = f.read()
...
def generate_selector(tree_data: dict, steps: int, output_dir: str) -> None:
    ...
    for tree_name, data in tree_data.items():
        ...
        metrics["confidence_hist"] = (
            HistogramGenerator.generate_confidence_histogram(
                metrics["confidences"]
            )
        )
        tree_metrics[tree_name] = metrics

    total_level_hist = HistogramGenerator.generate_level_histogram(
        total_level_counts, steps
    )
    total_confidence_hist = HistogramGenerator.generate_confidence_histogram(
        all_confidences
    )
