...
class NodeProcessor:
    ...
    def create_node_props(self, node, data: dict) -> dict:
        ...
        if data['level'] > 0:
            props.update({
                ...
                'shapeProperties': {"useImageSize": False, "useBorderWithImage": True},
                'color': {
                    'border': confidence_score_to_color(data['confidence']),
                    'borderWidth': 5
                },
            })
    ...
    def process_node_batch(self, nodes_batch) -> list[dict]:
        return [
            self.create_node_props(node, data)
            for node, data in nodes_batch
        ]


class PredictionTreeRenderer(Network):
    def __init__(self, tree: PredictionTree, n_workers=4, batch_size=100):
        super().__init__(height="600px", width="1500px", directed=True, notebook=False)
    ...
    def _process_nodes(self) -> list[dict]:
        ...
        with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
            ...
            for i in range(0, len(nodes_data), self.batch_size):
                ...
                futures.append(executor.submit(self.node_processor.process_node_batch, batch))
            
            for future in tqdm(futures, desc=f'Processing nodes for {self.tree.name}', leave=False):
    ...
    def _process_edges(self) -> None:
        ...
        for i in tqdm(range(0, len(edges_data), self.batch_size),
    ...
    def _add_nodes_to_network(self, nodes_data: list[dict]) -> None:
        for node_props in tqdm(nodes_data, desc=f'Adding nodes for {self.tree.name}', leave=False):
    ...
    def _calculate_tree_metrics(self, tree: PredictionTree) -> dict:
        ...
        confidences = [data['confidence'] for _, data in nodes_data if 'confidence' in data]
