...
class ErrorRates:
    ...
    def on_test_step(self) -> None:
        self.error_rates[self.step] = self.error_rates.get(self.step, 0) + int(self.current_pred)
    ...
    def result(self) -> float:
        result = {
            step: round((self.episodes - self.error_rates[step])/ self.episodes, 4)
...
class NodesCount:
    ...
    def result(self) -> int:
        ...
        trees = agent.trees_by_pred.values()
