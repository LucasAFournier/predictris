...
class PredictionTree(nx.DiGraph):
    ...
    def update_prediction(
        self, entry_node: UUID, context: Context, correct_pred: bool
    ):
        """Update prediction tree based on context and correctness of prediction."""
        entry_node = self.nodes[entry_node]
        if not correct_pred:
            entry_node["confident"] = False
        elif not entry_node["confident"]:
            if entry_node["level"] < self.depth:
                self.reinforce_correct_prediction(entry_node, context)
