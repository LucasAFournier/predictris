...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Tetris agent and collect prediction error rate data')
    ...
    parser.add_argument('--points', nargs='?', type=int, required=False, default=100, help='Number of measures for error rates. Default: 100 if flag is present.')
    ...
    parser.add_argument('--reps', type=int, required=False, default=1, help='Number of repetitions')
...
def test_episode(env: TetrisEnvironment, agent: Agent, tetrominos: list, action: str, activation: str):
    """Run a single test episode and collect metrics by step depth."""
    env.random_init(tetrominos)
    agent.init_test_episode(action, activation)

    for _ in range(TEST_STEPS):
        agent.test()


def collect_data(env: TetrisEnvironment, agent: Agent, interval: int, args, pbar):
    ...
    while total_steps < args.total_steps:
        ...
        if total_steps >= next_update:
            for _ in range(TEST_EPISODES):
                test_episode(env, agent, args.tetrominos, args.action, args.activation)
...
def main():
    ...
    for rep in range(args.reps):
        ...
        agent = env.build_agent(depth=args.depth, verbose=args.verbose, metrics=["error_rate"])

        with tqdm(total=args.total_steps, desc=f"[{rep + 1}/{args.reps}] {dir_name}", position=0, leave=args.verbose) as pbar:
    ...
    if args.plot:
        ...
        total_steps_data = [step for step, _ in error_rates_reps[0]] if error_rates_reps else []
        # error_rates_data is now a list of lists of dictionaries.
        error_rates_data = [[error_dict for _, error_dict in rep_data] for rep_data in error_rates_reps]

        plot_error_rates(
            steps_data=[total_steps_data],  # Wrap in a list for experiment grouping
            error_rates_data=[error_rates_data], # Wrap in a list for experiment grouping
            legends=[dir_name],
            output_path=Path(f'plots/{dir_name}/error-rates.png') if args.save else None
        )
