...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot nodes count data from CSV files"
    )
    parser.add_argument(
        "--dirs",
        type=str,
    ...
    parser.add_argument(
        "--metrics",
        nargs="+",
        default=["nodes_count", "confidences", "time_per_step"],
...
if __name__ == "__main__":
    ...
    if not data_series:
        ...
        output_path = (
            Path("plots/metrics") / args.output if args.output else None
        )
        plotter = PlotMetrics(
            data_series, labels, format_title(common_params), output_path
        )
