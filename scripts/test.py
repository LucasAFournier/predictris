...
def parse_args():
    ...
    parser.add_argument('--episode', type=str, help='Number of actions per episode or "auto"')
    ...
    parser.add_argument('--time', type=int, help='Time limit in seconds')
...
def main():
    ...
    steps_auto = (args.episode == "auto")
    current_steps = 2 if steps_auto else int(args.episode)
    ...
    while nodes_count < total:
        now = time.time()
        if args.time and now - start_time >= args.time / 2:
            if args.save and checkpoints:
                print(agent.perception_dict[0].__self__.name)
        if args.time and now - start_time >= args.time:
            print(f"\nTime limit of {args.time} seconds reached. Stopping...")
            if args.save and checkpoints:
                params = {
