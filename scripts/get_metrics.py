...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Tetris agent and collect nodes count data"
    )
    parser.add_argument(
        "--tetrominos",
        type=str,
    ...
    parser.add_argument(
        ...
        const=100,
        default=None,
        help="Number of measures for nodes count. Default: 100 if flag is present.",
    )
    parser.add_argument(
        "--confidences",
        nargs="?",
        ...
        const=4,
        default=None,
        help="Number of measures for confidences. Default: 4 if flag is present.",
    )
    parser.add_argument(
        "--time-per-step",
        nargs="?",
        ...
        const=100,
        default=None,
        help="Number of measures for time per step. Default: 100 if flag is present.",
    )
...
def collect_metrics(
    env: TetrisEnvironment, agent: Agent, interval, args, pbar
):
    """Collect nodes count data during learning."""
    metrics = {metric: [] for metric in interval}
    total_steps = 0
    next_update = {metric: 0 for metric in interval}
    while total_steps < args.total_steps:
        learn_episode(
            env, agent, args.tetrominos, args.action, args.activation
        )
        ...
        for metric in metrics:
            if total_steps >= next_update[metric]:
                metrics[metric].append(
                    (total_steps, agent.metrics[metric].result())
                )
...
def main():
    ...
    agent = env.build_agent(
        depth=args.depth, verbose=args.verbose, metrics=chosen_metrics
    )
    ...
    if args.plot:
        ...
        output_path = (
            Path(f"plots/{dir_name}/metrics.png") if args.save else None
        )
