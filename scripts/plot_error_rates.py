...
def parse_args():
    parser = argparse.ArgumentParser(description='Plot best prediction error rate from experiment data')
...
def extract_error_rates(directory: Path, step: int) -> Tuple[List[List[float]], List[List[float]]]:
...
if __name__ == '__main__':
    ...
    for exp_dir in args.dirs:
        total_steps_hist, error_rates_hist = extract_error_rates(Path('plots') / exp_dir / Path('error-rates'), args.step)
    ...
    output_path = Path('plots/error-rates') / args.output if args.output else None
    if output_path:
        output_path.parent.mkdir(parents=True, exist_ok=True)
    plot_error_rates(total_steps_data, error_rates_data, legends, HLINE_Y, output_path)